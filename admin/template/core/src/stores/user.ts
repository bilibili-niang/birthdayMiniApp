...
import { SCOPE_CS } from '@kacat/config'
...
const useUserStore = defineStore('USER', () => {
  ...
  const getUserInfo = async () => {
    try {
      const res = await request({
        url: '/kacat-cornerstone-system/me',
        method: 'get'
      })
      userInfo.value = res.data
      getMerchantInfo(userInfo.value!.merchantId)
    } catch (err) {
  ...
  const getMerchantInfo = (merchantId: string | number) => {
    request({
      url: `/kacat-cornerstone-merchant/merchant/${merchantId}`
    })
      .then((res) => {
        merchantInfo.value = res.data
        if (useAppStore().scope === SCOPE_CS) {
