...
const useBasicLayoutStore = defineStore('basic-layout', () => {
  ...
  const initMenu = () => {
    ...
    isRoutesError.value = false
    getHeaderMenus()
      .then((res) => {
        headerMenus.value = res.data
        const scene = import.meta.env.VITE_APP_SCENE || location.pathname.split('/')[1]
        console.log(`%c scene：${scene}`, 'color:#22c55e')
        const target = flattenHeaderMenus.value.find((item: any) => item.value === scene)
        const id = target?.id
        if (id) {
          ...
          isRoutesError.value = true
          message.error('抱歉，您没有权限访问')
        }
      })
      .catch((err) => {
