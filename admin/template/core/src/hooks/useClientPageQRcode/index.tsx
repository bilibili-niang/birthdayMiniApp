import { Empty, QRCode } from '@kacat/ui'
...
import { copyText } from '@kacat/utils'
...
const $getWeappQRCode = (options?: { page?: string; scene?: string; env?: 'release' | 'trial' | 'develop' }) => {
  ...
  return request({
    url: `/kacat-cornerstone-wechat-wap/m/${weappStore.isDirectWeapp ? 'direct' : 'open'}/miniapp/${
      weappId.value
    }/qrcode`,
    method: 'get',
    params: {
...
const $createShortLink = (originString: string) => {
  return request({
    url: '/kacat-cornerstone-system/short-url',
    method: 'post',
    data: {
      originString,
