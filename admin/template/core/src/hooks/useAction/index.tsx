import { defineComponent, ref, computed, type PropType, reactive } from 'vue'
import { Button, ConfigProvider, Empty, Input, Modal, message } from '@kacat/ui'
...
import { type ObjectSchema, type Schema, SchemaForm } from '@kacat/jsf'
...
export const ActionModal = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const currrentCate = computed(() => {
      return menus.value.find((item) => item.key === menuKey.value)
    })
      ...
      return (
        <div class={`${PREFIX_CLS}-action ${showPreview.value && 'show-preview'}`}>
            ...
            <div class={[`${PREFIX_CLS}-action__category`, 'scroller']}>
              {menus.value.map((item) => {
                return (
                  <div
                    class={[
                      'w_action__category-item scroller',
              ...
              {/* {menus.value.map((item) => {
                return (
                  <div
                    key={item.key}
                    class={[
                      `${PREFIX_CLS}-action__category-item clickable`,
                      menuKey.value === item.key && '--active'
                    ]}
                    onClick={() => (menuKey.value = item.key)}
                  >
                    <iconpark-icon class="icon" name={item.icon || 'pic'}></iconpark-icon>
          ...
          <div class={`${PREFIX_CLS}-action__content`}>
            {/* <div class={`${PREFIX_CLS}-action__content-header`}>
              <div class={`${PREFIX_CLS}-action__sub-title`}>{currrentCate.value?.name}</div>
            </div> */}
            <div class="w_action-modal__content">
              {/* <div class="w_action-modal__action-list scroller">
                {currrentCate.value?.actions.map((item) => {
                  const isActive = actionKey.value === item.key
                  return (
                    <div
                      class={['w_action-modal__item clickable', isActive && '--active']}
                      key={item.key}
                      onClick={() => toggleAction(item.key)}
                    >
                      <div class="w_action-modal__item-title">{item.title}</div>
                    </div>
                  )
                })}
              </div> */}
              {/* <div class="w_action__preview">
                <div class="w_action__preview-images"></div>
              </div> */}
              <div class="w_action-modal__configure">
                {/* <div class="w_action-modal__configure-header">{actionRef.value?.title}</div> */}
                {actionRef.value && (
                ...
                <div class="w_action-modal__configure-content scroller">
                  {actionKey.value ? (
                    <SchemaForm
                      theme="compact"
                      key={actionKey.value}
                  ...
                  {/* {actionRef.value?.schema ? (
                    <ThemeProvider theme={theme}>
                      <SchemaConfig
                        key={actionKey.value}
                        id={actionKey.value}
                        schema={actionRef.value.schema}
                    ...
                    <div class="w_action-modal__configure-empty">该动作没有参数配置</div>
                  ) : (
                    <div class="w_action-modal__configure-empty">尚未选择动作</div>
                  )} */}
                </div>
