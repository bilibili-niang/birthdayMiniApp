...
import { Button, Checkbox, ConfigProvider, Icon, message } from '@kacat/ui'
...
// const data = dv.data.sort((a, b) => a.id - b.id)
// const data = [] as any[]
...
export default defineComponent({
  ...
  setup(props, { expose, emit }) {
    ...
    onMounted(() => {
      try {
        const list = JSON.parse(localStorage.getItem('kacat-recent-images') || '[]')
    ...
    const toggleSpace = () => {
      ...
      state.currentCate = renderCategories.value[0]?.key
    }
    ...
    const onUploadSuccess = (image: ImageDefine) => {
      ...
      if (
        ...
        localStorage.setItem('kacat-recent-images', JSON.stringify(recentImages.value))
      }
    }
    const useUploadURL = () => {
            ...
            {!uploaderOnly.value && (
              <div class="lego-resource__menu">
                <div class="lego-resource__title">
                  <strong>设计资源</strong>
                  {/* <div class="lego-resource__space clickable" onClick={toggleSpace}>
                  <span>{state.isPersonal ? '我的空间' : '系统预置'}</span>
                  <Icon class="icon" name="toggler-1"></Icon>
                </div> */}
                </div>
                <div class="lego-resource__category scroller">
            ...
            <div class="lego-resource__content jsf-ui">
              <div class="lego-resource__content-header">
                <div class="lego-resource__sub-title">
                  {isUpload.value
                    ? // ? `上传资源 ${props.typeRef?.value} ${props.config?.value.multiple + ''}`
                      '上传资源'
