...
import type { TableProps } from '@kacat/ui'
import { Button, Checkbox, Icon, message, Modal, Pagination, TableSummary, Tooltip } from '@kacat/ui'
...
import { type ComputedValue, useBasicLayoutStore, useComputedValue, useModal } from '../../../lib'
...
const SearchTable = defineComponent({
  ...
  setup(props, { emit, attrs, slots }) {
    ...
    onMounted(() => {
      calcMaxWidth()
      try {
        wo.observe(document.querySelector('.kacat-basic-layout__router-view-content')!)
      } catch (e) {
        console.log('e:')
        console.log(e)
      }
...
export const useSearchTable = (config: SearchTableConfig, slots?: Record<string, any>) => {
  ...
  watch(
    ...
    () => {
      if (tableRef.value) {
        const el = tableRef.value?.querySelector('.kacat-table-body') as HTMLDivElement
...
const Comp = defineComponent({
  ...
  setup(props, { expose }) {
    ...
    /* 获取:
    const rightTableList = useTabSearchTable([...])
    rightTableList.component.exposed.current.value
    * */
    expose({
      current,
      toggle
    ...
    <div
      ...
      onClick={() => {
        useModal({
          ...
          centered: true,
          content: (
            <div class="base-table-modal-long-text scroller">
...
const usePreviewImages = (images: string[]) => {
  Modal.open({
    title: '图片列表',
    content: (
      <div class="base-table-images-preview scroller">
        {images.map((item) => {
          return <img src={item} />
        })}
      </div>
