import { KeepAlive, computed, defineComponent, onBeforeUnmount, onMounted, ref, watch } from 'vue'
...
import { Badge, Empty, Icon, Modal, Popover, Spin, Tooltip, message } from '@kacat/ui'
...
import { LOGO_URL, PREFIX_CLS, SCOPE_SU } from '@kacat/config'

import defaultAvatar from '../../../assets/default-avatar.svg'
import uuid from '../../../utils/uuid'
import router, { Exception404 } from '../../../router'
import { authMerchantDirect, authMerchantSwitch, getSwitchableMerchantList } from '../../../api/login'
import useRequestErrorMessage from '../../../hooks/useRequestErrorMessage'
import { renderAnyNode } from '@kacat/utils'

const exclusion = ['CommonDeckPage']

const getRouterView = (key?: any) => {
  return (
    <RouterView class={`${PREFIX_CLS}-basic-layout__router-view-content keep-alive scroller`}>
      {(scope: any) => {
...
const BasicLayout = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const refreshTaskCount = () => {
      getBackgroundTasks({ current: 1, size: 10 }).then((res) => {
    ...
    const CurrentApp = () => {
      ...
      return (
        <div class="basic-layout-brand current-app">
          ...
          <div class="basic-layout-brand__text current-app">{merchantInfo?.value?.menuName || '卡猫合作商后台'}</div>
        </div>
      )
    }
    ...
    return () => (
      <div class="toggle-merchant">
        <Spin spinning={isLoading.value}>
          ...
          <div class="toggle-merchant__list scroller">
