import { Button, Icon, JsonView, message, RadioButton, Select, Switch, Table, TabPane, Tabs, Tooltip } from '@kacat/ui'
...
import { CommonWidgetPropsDefine } from '@kacat/jsf'
...
import { $getStadiumList } from '@kacat/cs-venue/src/api/stadium'
import useSchemaFormModal from '../../../hooks/useSchemaFormModal'
import { CommonSelectorPropsDefine } from '../../../hooks/useCommonSelector'
import { defaults, divide, flatMap, flatten, uniq } from 'lodash'
import { onMounted } from 'vue'
import { $getPayeeConfig, $updatePayeeConfig } from '../api'
import { watch } from 'vue'
import { useResponseMessage } from '../../../hooks/useRequestErrorMessage'
import { requestGetShopList } from '../../../../../../apps/cs/microstore/src/api/shop'
...
const PayeeRuleOptions = [
  { label: '统一收款', value: PayeeRule.Union, desc: '统一收款：所有门店产生的收入都会进入到同一个收款账户中' },
  {
    label: '多账户收款',
...
export const CommonPaymentAccountConfig = defineComponent({
  setup() {
    ...
    const cashierEnable = computed(() => [SCENE_VENUE, SCENE_SHOP_HUB].includes(appStore.scene))
...
const PaymentConfig = defineComponent({
  ...
  setup(props) {
    ...
    const getConfig = () => {
      const end = message.loading('加载中...')
      $getPayeeConfig({ productCode: useAppStore().scene, payScene: props.payScene })
        .then((res) => {
          if (res.code === 200) {
    ...
    const format = () => {
      const { productCode, payScene, storeValueCard, receivingType, unifiedRule, multipleRule } = state.value
        ...
        <div class="p_payment-config">
          <div class="p_payment-config__form">
            <div class="value-card">
              ...
              <div class="color-disabled">关闭后用户前台的确认下单页面不显示「储值卡付款」</div>
            </div>
            <br />
            <h2>收款规则</h2>
            <RadioButton
              style="width:200px"
              options={PayeeRuleOptions}
            ...
            <div class="color-disabled" style="margin-top:8px">
              {PayeeRuleOptions.find((item) => item.value === state.value.receivingType)?.desc}
            </div>
...
const usePayMethod = (options?: {
  ...
  const payMethods = computed(() => {
    return SUB_PAYMENT_OPTIONS.map((item) => {
      ...
      const paySceneDisabled = !paySceneSelectablePayMethods.value.includes(item.value)

      return {
        ...item,
        label: paySceneDisabled ? `${item.label}（支付场景不适用）` : disabled ? `${item.label}（已添加）` : item.label,
