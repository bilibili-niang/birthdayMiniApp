...
import { CommonSelectorPropsDefine, useSearchTable, useTableImages } from '@kacat/core'
import { Button, Switch } from '@kacat/ui'
import { COMMON_STATUS_ON, COMMON_STATUS_OPTIONS } from '@kacat/config'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getVenues = () => {
      ...
      // })
      //   .then((res) => {
      //     venueOptions.value = res.data.records.map((item) => {
      //       return {
      //         label: item.name,
      //         value: item.id,
      //         disabled: !(item.gymList?.length > 0),
      //         children: (item.gymList || []).map((gym: any) => {
      //           return {
      //             label: gym.name,
      //             value: gym.id
      //           }
