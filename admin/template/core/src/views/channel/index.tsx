...
/**
 *（1）参考「卡猫精选H5」&「卡猫精选小程序」，商家装修的店铺数据可以同时输出H5版本以及小程序版本
 *（2）点击「前往创建」按钮需要判断：
 *  - 如果用户之前没有选择过小程序注册场景，应该跳转  02-1 选择小程序注册场景页面
 *  - 如果用户已经选择某种场景注册过小程序，应该直接进入其选择的场景的配置步骤页面
 *（3）如果已经发布了小程序，「前往创建」文案改成「前往查看」
 */
...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const onWxOaAuth = () => {
      ...
      }/kacat-cornerstone-wechat/open/authUrl?merchant_id=${useUserStore().userInfo?.merchantId}&scene=${
        useAppStore().scene
      }`
    }
