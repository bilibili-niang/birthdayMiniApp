...
export default defineComponent({
  ...
  setup() {
        ...
        content: (
          <div>
            {useSearchTable({
              key: 'weapp-sign-oa',
              title: '选择已授权公众号',
              requestURL: '/kacat-cornerstone-wechat/account?type=0',
              filter: {
    ...
    const authWeappOa = async (accountId: string) => {
      ...
      setTimeout(() => {
        window.open(
          res.data.replace('&redirect_uri=https%3A%2F%2Fdev-api.kacat.cn', '&redirect_uri=http%3A%2F%2Flocalhost:5173')
        )
        setTimeout(() => {
          Modal.confirm({
            title: '提示',
