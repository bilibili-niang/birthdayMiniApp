...
export const getHeaderMenus = (scope?: string) => {
  ...
  return request({
    url: '/kacat-cornerstone-system/menu/top-menu',
    method: 'GET',
    params: {
...
export const getSideMenus = (topMenuId: string) => {
  return request({
    url: `/kacat-cornerstone-system/menu/routesIncludeButton/${topMenuId}`
  }).then((res: any) => {
...
export const getSideMenusTree = (topMenuId: string) => {
  return request({
    // url: `/kacat-cornerstone-system/menu/tree/${topMenuId}`,
    url: `/kacat-cornerstone-system/menu/treeIncludeButton/${topMenuId}`
  }).then((res: any) => {
    if (res && Array.isArray(res.data)) {
      const sorted = sortMenuTree(res.data)
      // 返回一个新的响应对象，避免直接篡改原始引用
