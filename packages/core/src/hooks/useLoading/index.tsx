...
export const GlobalLoading = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return () => {
      return (
        <div class={['kacat-global-loading', (props.toastVisible || closeable.value) && 'toast-visible']}>
          <Spin primary={props.toastVisible || closeable.value} />
          <div class="kacat-global-loading__tip">{props.text}</div>
          {closeable.value && <Icon class="kacat-global-loading__close" name="close" onClick={close} />}
        </div>
      )
    }
  }
